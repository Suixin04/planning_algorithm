...
double d_euclid(Point2D p1, Point2D p2) {
...
bool AstarPlanner::plan(const GridMap &map,
    ...
    std::vector<std::vector<double>> heuristic(height, std::vector<double>(width, 0));

    for (int i = 0; i < width; ++i)
        for (int j = 0; j < height; ++j)
            heuristic[j][i] = d_euclid({i, j}, goal);
